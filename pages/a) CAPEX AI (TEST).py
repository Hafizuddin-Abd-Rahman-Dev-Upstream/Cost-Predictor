# ======================================================================================
# CAPEX AI RT2026 — FULL FIXED VERSION (Streamlit Cloud-ready)
#
# ✅ Fixes included:
# 1) Monte Carlo as its OWN top-level tab: 🎲 Monte Carlo
# 2) Fix StreamlitDuplicateElementId by giving ALL data_editor widgets unique keys
# 3) Tabs no longer look "empty" — they show guidance messages instead of stopping silently
# 4) Dependency guardrails: if scikit-learn (or others) missing, app shows clear message
#
# NOTE (IMPORTANT for Streamlit Cloud):
# - You MUST add required packages in requirements.txt (see bottom of this file).
# ======================================================================================
...
# -------------------------
# Dependency guards (so Cloud errors are clear)
# -------------------------
missing = []
...
if missing:
    ...
    st.error(
        "Missing Python packages in your Streamlit environment.\n\n"
        "Add these to requirements.txt:\n"
        f"- " + "\n- ".join(missing)
    )
    st.info(
        "On Streamlit Cloud: open your repo → add/update requirements.txt → redeploy.\n\n"
        "Example requirements.txt is included at the bottom of this script."
    )
    st.stop()
...
with tab_data:
    ...
    if data_source == "Upload CSV":
        ...
        if github_csvs:
            selected_file = st.selectbox("Choose CSV from GitHub", github_csvs, key="gh_csv_select")
            if st.button("Load selected CSV", key="gh_csv_load_btn"):
    ...
    if not st.session_state.datasets:
        st.warning("Upload or load a dataset to proceed.")
    else:
        ds_name_data = st.selectbox("Active dataset", list(st.session_state.datasets.keys()), key="active_dataset_select")
        ...
        if len(num_cols) < 2:
            ...
            with m1:
                test_size = st.slider("Test size", 0.1, 0.5, 0.2, 0.05, key="test_size_slider")
            ...
            with st.expander("Visualization settings", expanded=False):
                use_knn = st.checkbox("Use KNN imputation for visualization (slower)", value=False, key="viz_knn_chk")
            ...
            try:
                ...
                if target_col_viz not in num_imputed.columns:
                    ...
                    feat = st.selectbox("Select feature for cost curve", list(X_viz.columns), key="cost_curve_feat")
...
# =======================================================================================
with tab_pb:
    st.markdown('<h3 style="margin-top:0;color:#000;">🏗️ Project Builder</h3>', unsafe_allow_html=True)
    st.caption("Assemble multi-component CAPEX projects")

    if not st.session_state.datasets:
        st.warning("No dataset loaded. Go to **📊 Data** tab and upload/load a CSV first.")
    else:
        ...
        if not st.session_state.projects:
            ...
            # ✅ FIX: Unique data_editor key to prevent DuplicateElementId
            comp_edited = st.data_editor(
                comp_row_df,
                num_rows="fixed",
                use_container_width=True,
                key=f"pb_editor__{proj_sel}__{dataset_for_comp}__{st.session_state.widget_nonce}",
            )
            ...
            if st.button("➕ Predict & Add Component", key=f"pb_add_comp_{proj_sel}_{dataset_for_comp}"):
                try:
                    ...
                    # ✅ Force widget identity refresh after structural changes
                    st.session_state.widget_nonce += 1
...
with tab_mc:
    ...
    if not st.session_state.projects:
        st.warning("No projects found. Create a project first in **🏗️ Project Builder**.")
    else:
        ...
        comps = proj.get("components", [])
        if not comps:
            st.info("This project has no components. Add components in **🏗️ Project Builder** first.")
        else:
...
with tab_compare:
    ...
    st.caption("Portfolio-level CAPEX view")

    proj_names = list(st.session_state.projects.keys())
    if len(proj_names) < 2:
        st.warning("Create at least **two** projects in **🏗️ Project Builder** to compare.")
    else:
        ...
        if len(compare_sel) < 2:
            st.info("Select at least two projects for a meaningful comparison.")
        else:
            rows = []
...
# ======================================================================================
# requirements.txt (PUT THIS IN A FILE NAMED requirements.txt IN YOUR REPO ROOT)
# ======================================================================================
# streamlit
# pandas
# numpy
