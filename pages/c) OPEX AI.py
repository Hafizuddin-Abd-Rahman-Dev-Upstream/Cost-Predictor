# ======================= OPEX AI — PETRONAS Gradient UI (Full App, with Project Builder + Exports) =======================
...
def create_project_excel_report_opex(project_name, proj, currency=""):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        for col_idx in range(3, max_col + 1):  # from "Base OPEX" onwards
...
def create_comparison_excel_report_opex(projects_dict, currency=""):
    ...
    for name, proj in projects_dict.items():
        ...
        capex = dfc["Base OPEX"].sum() if not dfc.empty else 0.0
...
with tab_data:
    ...
    with c2:
        ...
        data_link = ("https://petronas.sharepoint.com/sites/ecm_ups_coe/confidential/DFE%20Cost%20Engineering/Forms/AllItems.aspx?id=%2Fsites%2Fecm%5Fups%5Fcoe%2Fconfidential%2FDFE%20Cost%20Engineering%2F2%2ETemplate%20Tools%2FCost%20Predictor%2FDatabase%2FOPEX%20AI&viewid=25092e6d%2D373d%2D41fe%2D8f6f%2D486cd8cdd5b8"
        )
        st.markdown(
    ...
    if not st.session_state.datasets:
        ...
        if preds:
            ...
            st.download_button(
                ...
                file_name=f"{ds_name}_opex_all.zip",
...
with tab_pb:
    st.markdown(
        '<h4 style="margin:0;color:#000;">Project Builder</h4><p>Assemble multi-component OPEX projects</p>',
        unsafe_allow_html=True,
    )
    ...
    if not st.session_state.datasets:
        ...
        if not st.session_state.projects:
            ...
            if not comps:
                ...
                st.dataframe(
                    dfc.style.format(
                        {"Base OPEX": "{:,.2f}", "Grand Total": "{:,.2f}"}
                    ),
                    use_container_width=True,
                )
                total_capex = float(sum(r["Base OPEX"] for r in rows))
                ...
                with col_t1:
                    st.metric("Project OPEX", f"{curr} {total_capex:,.2f}")
                with col_t2:
                    st.metric(
                        "Project Grand Total",
                ...
                with col_dl1:
                    excel_report = create_project_excel_report_opex(
                        proj_sel, proj, curr
                    )
                    st.download_button(
                ...
                with col_dl2:
                    pptx_report = create_project_pptx_report_opex(
                        proj_sel, proj, curr
                    )
                    st.download_button(
...
with tab_compare:
    st.markdown(
        '<h4 style="margin:0;color:#000;">Compare Projects</h4><p>Portfolio-level OPEX view</p>',
        unsafe_allow_html=True,
    )
    ...
    if len(proj_names) < 2:
        ...
        if len(compare_sel) < 2:
            ...
            st.dataframe(
                df_proj[
                    ["Project", "Components", "OPEX Sum", "Grand Total"]
                ].style.format(
            ...
            with col_c1:
                excel_comp = create_comparison_excel_report_opex(
            ...
            with col_c2:
                pptx_comp = create_comparison_pptx_report_opex(
                    projects_to_export, currency_comp
                )
                st.download_button(
