...
def create_project_excel_report_OPEX(project_name, proj, currency=""):
    ...
    total_OPEX = comps_df["Base OPEX"].sum()
...
def create_project_pptx_report_OPEX(project_name, proj, currency=""):
    prs = Presentation()
    ...
    comps_df = project_components_df(proj)
    comps = proj.get("components", [])
    total_OPEX = comps_df["Base OPEX"].sum() if not comps_df.empty else 0.0
...
def create_comparison_excel_report_OPEX(projects_dict, currency=""):
    output = io.BytesIO()

    summary_rows = []
    for name, proj in projects_dict.items():
        dfc = project_components_df(proj)
        OPEX = dfc["Base OPEX"].sum() if not dfc.empty else 0.0
...
def create_comparison_pptx_report_OPEX(projects_dict, currency=""):
    prs = Presentation()
    prs.slide_width = Inches(10)
    prs.slide_height = Inches(7.5)
...
with tab_data:
    ...
    with c2:
        ...
        data_link = (
            ...
            "%2F2%2ETemplate%20Tools%2FCost%20Predictor%2FDatabase%2FOPEX%20%2D%20RT%20Q1%202025"
    ...
    if not st.session_state.datasets:
        ...
        if preds:
            ...
            st.download_button(
                ...
                file_name=f"{ds_name_res}_OPEX_all.zip",
...
# =======================================================================================
with tab_pb:
    st.markdown('<h4 style="margin:0;color:#000;">Project Builder</h4><p>Assemble multi-component OPEX projects</p>', unsafe_allow_html=True)
    ...
    if not st.session_state.datasets:
        ...
        with colA:
            new_project_name = st.text_input("New Project Name", placeholder="e.g., OPEX 2026", key="pb_new_project_name")
        with colB:
        ...
        if not st.session_state.projects:
            ...
            if st.button("➕ Predict & Add Component", key=f"pb_add_comp_{proj_sel}_{dataset_for_comp}"):
                ...
                try:
                    ...
                    st.error(f"Failed to predict component OPEX: {e}")
            ...
            if not comps:
                ...
                st.dataframe(
                    dfc.style.format({"Base OPEX": "{:,.2f}", "Grand Total": "{:,.2f}"}),
                    use_container_width=True,
                )

                total_OPEX = float(dfc["Base OPEX"].sum())
                total_grand = float(dfc["Grand Total"].sum())
                proj["totals"] = {"OPEX_sum": total_OPEX, "grand_total": total_grand}

                col_t1, col_t2 = st.columns(2)
                with col_t1:
                    st.metric("Project OPEX", f"{curr} {total_OPEX:,.2f}")
                with col_t2:
                ...
                with col_dl1:
                    excel_report = create_project_excel_report_OPEX(proj_sel, proj, curr)
                    st.download_button(
                        "⬇️ Download Project Excel",
                        data=excel_report,
                        file_name=f"{proj_sel}_OPEX_Report.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    )

                with col_dl2:
                    pptx_report = create_project_pptx_report_OPEX(proj_sel, proj, curr)
                    st.download_button(
                        "⬇️ Download Project PowerPoint",
                        data=pptx_report,
                        file_name=f"{proj_sel}_OPEX_Report.pptx",
...
# =======================================================================================
with tab_compare:
    st.markdown('<h4 style="margin:0;color:#000;">Compare Projects</h4><p>Portfolio-level OPEX view</p>', unsafe_allow_html=True)
    ...
    if len(proj_names) < 2:
        ...
        if len(compare_sel) < 2:
            ...
            for p in compare_sel:
                ...
                OPEX = float(dfc["Base OPEX"].sum()) if not dfc.empty else 0.0
            ...
            st.dataframe(
                df_proj[["Project", "Components", "OPEX Sum", "Grand Total"]].style.format(
                    {"OPEX Sum": "{:,.2f}", "Grand Total": "{:,.2f}"}
                ),
                use_container_width=True,
            )
            ...
            with col_c1:
                excel_comp = create_comparison_excel_report_OPEX(projects_to_export, currency_comp)
                st.download_button(
                    "⬇️ Download Comparison Excel",
                    data=excel_comp,
                    file_name="OPEX_Projects_Comparison.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                )

            with col_c2:
                pptx_comp = create_comparison_pptx_report_OPEX(projects_to_export, currency_comp)
                st.download_button(
                    "⬇️ Download Comparison PowerPoint",
                    data=pptx_comp,
                    file_name="OPEX_Projects_Comparison.pptx",
                    mime="application/vnd.openxmlformats-officedocument.presentationml.presentation",
                )
