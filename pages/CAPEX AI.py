...
correct_password = st.secrets.get("password_capex", None)

if not st.session_state.authenticated:
    with st.form("login_form"):
        password = st.text_input("🔐 Enter Access Password", type="password_capex")
...
def main():
    ...
    if num_features <= 5:
        ...
        # If more than 5 features, create multiple rows
        cols = []
        for i in range(0, num_features, 5):
            row_cols = st.columns(min(5, num_features - i))
